...
import { LoginRequest, LoginResponse, lastedAppVersionResponse} from "./evento.types"

export const login = async ({ username, password }: LoginRequest): Promise<LoginResponse> => {
  try {
    const response = await httpClient.post('auth_login_app?app-mobile=true&v=' + Date.now(), { username, password })
...
export const getLastedAppVersion = async (): Promise<lastedAppVersionResponse> => {
  try {
    const response = await httpClient.get('getLastedAppVersion/cashless?v=' + Date.now());
